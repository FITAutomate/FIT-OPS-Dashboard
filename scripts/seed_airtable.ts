/**
 * Airtable Demo Data Seeding Script
 * 
...
async function seedCompanies(): Promise<CreatedRecord[]> {
  console.log('\n📦 Creating Companies...');
  ...
  for (const company of DEMO_COMPANIES) {
    try {
      ...
      console.log(`  ✅ Created company: ${company.name} (${record.id})`);
...
async function seedContacts(companyIds: CreatedRecord[]): Promise<CreatedRecord[]> {
  console.log('\n👥 Creating Contacts...');
  ...
  for (const contact of DEMO_CONTACTS) {
    try {
      ...
      if (existing.length > 0) {
        console.log(`  ⏭️  Contact "${contact.firstName} ${contact.lastName}" already exists, skipping`);
      ...
      console.log(`  ✅ Created contact: ${contact.firstName} ${contact.lastName} (${record.id})`);
...
async function seedProjects(contactIds: CreatedRecord[]): Promise<void> {
  console.log('\n📋 Creating Projects...');
  ...
  for (const project of DEMO_PROJECTS) {
    try {
      ...
      const record = await base('Projects').create(fields);

      console.log(`  ✅ Created project: ${project.name} (${record.id})`);
    } catch (error: any) {
...
async function main() {
  console.log('🌱 Starting Airtable Demo Data Seeding...');
  console.log('=========================================\n');

  try {
    const companies = await seedCompanies();
    console.log(`\n   Created/Found ${companies.length} companies`);

    const contacts = await seedContacts(companies);
    console.log(`\n   Created/Found ${contacts.length} contacts`);

    await seedProjects(contacts);
